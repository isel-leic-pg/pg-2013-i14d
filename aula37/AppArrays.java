...
public class AppArrays{	
	...
	{	
		...
		char[] charsToPrint = new char[lineChars.length];
		int[] charsOccurrences = new int[lineChars.length];
		int idx = 0;
		...
		for(int i = 0; i < lineChars.length; i++){
			char c = lineChars[i];
			if(ArraysUtils.contains(charsToPrint, c) == false){
				charsToPrint[idx] = c;
				charsOccurrences[idx] = ArraysUtils.occurrences(charsToPrint, c);
				idx++;
			}
		}
		...
		for(int i = 0; i < charsOccurrences.length; i++){
			if(charsOccurrences[i] != 0){
				System.out.print(charsToPrint[i] + " ");
				Io.printChar(charsOccurrences[i], '*');
			}
		}
	}
